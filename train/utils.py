...
def crop_brain_region(file_path_list):
    
    volume_list = []
    volume_t1 = load_nifty_volume_as_array(file_path_list[0]) #t1 img
    original_shape = volume_t1.shape
    margin = 5 # small padding value
    bbmin, bbmax = get_none_zero_region(volume_t1, margin)
    volume_t1 = crop_ND_volume_with_bounding_box(volume_t1, bbmin, bbmax)
    volume_t1 = itensity_normalize_one_volume(volume_t1)
    volume_list.append(volume_t1)
    weight = np.asarray(volume_t1 > 0, np.float32)
    
    for path in file_path_list[1:-1]:
        volume = load_nifty_volume_as_array(path) #size :155 244 244
        volume = crop_ND_volume_with_bounding_box(volume, bbmin, bbmax)
        volume = itensity_normalize_one_volume(volume)
        volume_list.append(volume)
        

    label = load_nifty_volume_as_array(file_path_list[-1])
    label[label == 4] = 3
...
def get_roi(img, mask):
    ...
    sub_data = []
    for im in img:
        sub_img = extract_roi_from_volume(im,
                                            center_point,
                                            configTrain.PATCH_SIZE,
                                            fill = 'zero')  
        sub_data.append(sub_img)
    
    axis = [1,2,3,0]
    sub_data  = np.transpose(sub_data, axis)
    sub_label  = np.transpose(sub_label[np.newaxis, ...], axis)    
    return  sub_data, sub_label

def image_gen3d(df, batch_size = configTrain.BATCH_SIZE):
    
    grade = df['Grade'].values
    name_id = df['BraTS_2019_subject_ID'].values

    all_batches = np.stack((grade, name_id), 1)   
    out_img = []
    out_mask = []
    ...
    while True:
        ...
        for data in all_batches: 
            ...
            file_path_list = []
            for name in configTrain.name_list:
                file_path_list.append(os.path.join(configTrain.fold_train, grade_img, name_img, name_img + '_' + name + '.nii.gz' ))
            
            img, mask = crop_brain_region(file_path_list)
            img, mask = get_roi(img, mask)
            
            if configTrain.DATA_SAMPLING == 'all_positive':
                if mask.sum() == 0:
                    continue
            elif configTrain.DATA_SAMPLING == 'one_positive':
                if len(out_img) == batch_size - 1:
                    if sum(out_mask) == 0:
                        continue
