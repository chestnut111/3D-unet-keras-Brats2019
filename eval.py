...
def segment_one_image_dynamic(data, create_model_func):
    ...
    final_probs = set_ND_volume_roi_with_bounding_box_range(final_probs, temp_bbox[0]+[0], temp_bbox[1]+[1], prob1)
...
#     pred1[pred1 == 2] = 4
    ...
    final_probs = np.zeros(list(temp_size) + [config.NUM_CLASS], np.float32)
    final_probs = set_ND_volume_roi_with_bounding_box_range(final_probs, temp_bbox[0]+[0], temp_bbox[1]+[2], prob1) #temp_bbox[1]+[3]
...
def pred_brats(df, detect_func):
    ...
    with tqdm.tqdm(total=df.size(), **get_tqdm_kwargs()) as pbar:
        for filename, image_id, data in df.get_data():
#             final_label = detect_func(data)
            final_label, probs = detect_func(data)
            if config.TEST_FLIP:
#                 pred_flip = detect_func(flip_lr(data))
                pred_flip, probs_flip = detect_func(flip_lr(data))
                final_prob = (probs + probs_flip) / 2.0
                pred = np.argmax(final_prob, axis=-1)
#                 pred[pred == 2] = 4
            ...
            save_to_nii(final_label, image_id, outdir=config.OUT, mode="label")
